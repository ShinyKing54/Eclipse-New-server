...
exports.commands = {
    ...
    poof: function(target, room, user) {
            ...
            var poof = JSON.parse(fs.readFileSync('poof.json'));
	...
	if (global.poofoff) return this.sendReply("Poof is currently disabled.");
	if (!target) return this.sendReply("You need to specify a custom poof message.");
	this.add('|html|<center><b><font color = "' + randomcolor() + '">~~' + user.name + ' ' + target);
    user.disconnectAll();
	},
    addpoof: function(target, room, user) {
        ...
        if (this.can('hotpatch') || user.buypoof) {
            ...
            fs.writeFile('poof.json', JSON.stringify(poof, null, 1));
